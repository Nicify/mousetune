...
extern int getPointerResolution() {
  ...
  assert(KERN_SUCCESS == IOHIDGetParameter(hdl, CFSTR(kIOHIDPointerResolutionKey), (IOByteCount)sizeof(res), &res, &resByteSize));
...
extern int getMouseAcceleration() {
  ...
  assert(KERN_SUCCESS == IOHIDGetParameter(hdl, CFSTR(kIOHIDMouseAccelerationType), (IOByteCount)sizeof(acc), &acc, &accByteSize));
